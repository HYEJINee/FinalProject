...
<body>
	<!-- pageInfo -->
	<c:set var="numPerPage" value="6" />
	<!-- 각 페이지마다 게시물의 수 -->
	<c:set var="pagePerBlock" value="5" />
	<!-- 페이지블럭 마다 페이지의 수 -->
	<c:set var="nowPage" value="${param.nowPage}" />
	<!-- 현재 페이지 위치 -->
	<c:set var="nowBlock" value="${param.nowBlock}" />
	<!-- 현재 블럭 위치 -->
	<c:set var="totalPage" value="${main.size()/numPerPage}" />
	<fmt:formatNumber var="totalPage"
		value="${totalPage+(1-(totalPage%1))%1}" maxFractionDigits="0" />
	<!-- ceiling -->
	<c:set var="totalBlock" value="${totalPage/pagePerBlock}" />
	<fmt:formatNumber var="totalBlock"
		value="${totalBlock+(1-(totalBlock%1))%1}" maxFractionDigits="0" />
	<!-- ceiling -->
	...
	<div class="container  main_container">
		<div class="row">
			<div class="col-md-12">
				<h3 class="list_title background">
					<span>토론중</span>
				</h3>
				<c:choose>
					...
					<c:otherwise>
						<c:forEach items="${main}" var="mainList" begin="${beginPerPage}"
							end="${beginPerPage + numPerPage -1}">
							<c:choose>
								<c:when test="${mainList.topic_type eq 0}">
									<div class="card card--big grid-item col-md-3 col-md-offset-1"
										style="padding-left: 0px; padding-right: 0px; width: 31%; margin-left: 2%;">
										<input type="hidden" name="topic_no"
											value="${mainList.topic_no}" />
										<!-- card_header -->
										<!-- 1) 커버이미지 있을 때 -->
										...
										<div class="card_body">
											<p class="card__text">${mainList.topic_short_cont}</p>
										</div>
										<div class="card-footer card__action-bar">
											<p class="text-right">
											<span class="glyphicon glyphicon-user">
												${mainList.debate_tot_pro + mainList.debate_tot_con + mainList.debate_tot_neut}</span>
											<c:choose>
												...
												<c:otherwise>
													<span class="pro"> 찬 <fmt:formatNumber
															value="${mainList.debate_tot_pro/(mainList.debate_tot_pro + mainList.debate_tot_con + mainList.debate_tot_neut)*100}"
															pattern=".0" />%
													</span>
												</c:otherwise>
											</c:choose>
											<c:choose>
												...
												<c:otherwise>
													<span class="con"> 반 <fmt:formatNumber
															value="${mainList.debate_tot_con/(mainList.debate_tot_pro + mainList.debate_tot_con + mainList.debate_tot_neut)*100}"
															pattern=".0" />%
													</span>
												</c:otherwise>
											</c:choose>
											<c:choose>
												...
												<c:otherwise>
													<span class="neut"> 중 <fmt:formatNumber
															value="${mainList.debate_tot_neut/(mainList.debate_tot_pro + mainList.debate_tot_con + mainList.debate_tot_neut)*100}"
															pattern=".0" />%
													</span>
												</c:otherwise>
											</c:choose>
					...
					<nav>
						<ul class="pagination">
							<c:if test="${nowBlock > 0}">
								<li><a
									href="/?nowBlock=${nowBlock-1}&nowPage=${(nowBlock-1)*pagePerBlock}"
							...
							<c:forEach begin="0" end="${pagePerBlock-1}" varStatus="status">
								<c:if test="${not doneLoop}">
									<c:choose>
										<c:when test="${nowPage eq null && status.current eq 0}">
											<li class="active"><a
												href="/?nowBlock=${nowBlock}&nowPage=${status.current + (nowBlock*pagePerBlock)}">${status.current+1 + (nowBlock*pagePerBlock)}</a></li>
										</c:when>
										<c:when
											test="${status.current + (nowBlock*pagePerBlock) eq nowPage && nowPage ne null}">
											<li class="active"><a
												href="/?nowBlock=${nowBlock}&nowPage=${status.current + (nowBlock*pagePerBlock)}">${status.current+1 + (nowBlock*pagePerBlock)}</a></li>
										</c:when>
										<c:otherwise>
								...
								<c:if
									test="${status.current+1 + (nowBlock*pagePerBlock) eq totalPage}">
						...
						<c:if test="${not recomnd_loop}">
							<div class="item on_item">
								...
								<span class="board_title">${recmdList.topic_title}</span> <span
									class="board_icon glyphicon glyphicon-star">${recmdList.recomnd_cnt}</span>
							</div>
						</c:if>
							...
							<div class="item fin_item ">
								...
								<c:if test="${finishList.topic_type eq 0}">
									<c:choose>
										...
										<c:otherwise>
											<span class="pro_board"> 찬 <fmt:formatNumber
								...
								<c:if test="${finishList.topic_type eq 1}">
									<span class="board_icon glyphicon glyphicon-comment">
