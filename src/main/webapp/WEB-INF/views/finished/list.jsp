				...
				<c:forEach items="${list}" var="list">
					...
					<c:choose>
						<c:when test="${list.topic_type eq 0}">
							<h3><label class="label label-primary">찬반</label><br/></h3>
							<center><h3>${list.topic_title}</h3></center>
							<hr/>
							<p>${list.topic_short_cont}</p>
							<span class="glyphicon glyphicon-user"> ${list.debate_tot_pro + mainList.debate_tot_con + mainList.debate_tot_neut}</span>
							<c:choose>
