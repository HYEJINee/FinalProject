...
<body>
	...
	<div class="container top">
		<form class="form-horizontal">
			...
			<c:choose>
				<c:when
					test='${readlist.img_file_name != "" && readlist.img_file_name != null}'>
					<div id="divCoverImg" class="jumbotron">
						...
						<p id="short_cont">${readlist.topic_short_cont}<br>
						</p>
						<p id="writer" class="text-right">건의자 : ${readlist.user_nick}</p>
						<img id="coverImg" alt="커버 이미지"
							src="/resources/proposal/img/${readlist.img_file_name}.${readlist.img_ext_name}">
					</div>
				</c:when>
				<c:otherwise>
					<div id="divCoverImg" class="jumbotron"
										...
										<div>
											<c:choose>
												<c:when test="${empty readuser.user_nick != false}">
													...
													<div class="col-sm-offset-1 col-sm-1 minibottom">
														<button type="button" class="btn btn-default"
															id="btn${status.count}" onclick="Ref(this)"
															disabled="disabled">답글</button>
													</div>
												</c:when>
												<c:when test="${readuser.user_nick != opinion.user_nick}">
													...
													<div class="col-sm-offset-1 col-sm-1 minibottom">
														<button type="button" class="btn btn-default"
															id="btn${status.count}" onclick="Ref(this)">답글</button>
	...
	<nav id="page_nav" style="display: none">
		<p>
			<a href="/read/read?topic_no=${readlist.topic_no}&pageNo=2"></a>
		</p>
	</nav>
				...
				<c:when test="${readlist.topic_type eq 0}">
					<div class="form-group">
						...
						<div class="col-sm-1">
							<div class="profile-75">
								<c:choose>
								<c:when test="${readuser.user_profile != null}">
									<div class="round-profile"
										style="background-image: url(/resources/mypage/img/${readuser.user_profile})">
										<img class="user-lv-26" src="${readuser.user_lv_img}" />
									</div>
								</c:when>
								<c:otherwise>
									<div class="round-profile"
					...
					<div class="form-group">
						...
						<div class="col-sm-2">
							<h4 style="padding-top: 12px">에 대한 답글</h4>
						</div>
					</div>
				...
				<c:when test="${readlist.topic_type eq 1}">
					<div class="form-group">
						...
						<div class="col-sm-1">
							<c:choose>
								<c:when test="${readuser.user_profile != null}">
									<img src="/resources/mypage/img/${readuser.user_profile}"
										style="height: 80px; width: 80px" />
								</c:when>
								<c:otherwise>
						...
						<div class="col-sm-1">
							<c:choose>
								<c:when test="${readuser.user_lv_img != null}">
									<img src="${readuser.user_lv_img}"
	...
	<script
		src="${pageContext.request.contextPath}/resources/read/js/jquery.infinitescroll.min.js"></script>
	<script
